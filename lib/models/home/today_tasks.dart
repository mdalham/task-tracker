  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (mounted) {
        ...
        final indices = _generateBannerIndices(todayTasks.length, _indices);

        if (indices.isEmpty) {
          debugPrint('[TodayTasks] No banner indices generated');
          return;
        }
        ...
        debugPrint(
          '[TodayTasks] Banner manager initialized with ${indices.length} positions',
        );
      }
    });
  }
    ...
    return Consumer<TaskProvider>(
      builder: (context, provider, _) {
        ...
        if (todayTasks.isEmpty) {
          return EmptyState(title: 'No tasks for today');
        }

        if (provider.isLoading) {
          return LoadingSkeleton(
            loadingSkeletonItemCount: isExpanded ? todayTasks.length : 1,
          );
        }

        final displayTasks = isExpanded ? todayTasks : todayTasks.take(1).toList();
            ...
            GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Padding(
                  ...
                  child: _buildTaskList(displayTasks, cs, tt),
                ),
              ),
            ),
